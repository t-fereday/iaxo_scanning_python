...
# ---------------------------------------------------------------------------
# Default output directory for .sxd.npz surfaces — the repo's SXD/ folder,
# anchored to this package's location so it does not depend on the cwd.
# (Replaces IDL's hardcoded 'C:\NuSTAR\Scan\SXD\' / 'C:\IDL\...\SXD\' paths.)
_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SXD_DIR = os.path.join(_REPO_ROOT, 'SXD')

# VMfile (virtual-mounting) export directory.
# Edit this to change where the VMfile export is written; the explicit VMdir
# argument takes precedence, and a None falls back to the scan-data folder.
VMFILE_DIR = SXD_DIR
# ---------------------------------------------------------------------------
...
def surf_create(
    ...
    if os.path.isdir(_filedir):
        ...
        # basedir default was IDL's Windows scan dir; fall back to the cwd.
        indir = basedir if basedir else '.' + os.sep
    nofile = False
    ...
    # Write output
    # ------------------------------------------------------------------
    # Regular gridded-surface export. Suppressed on VMfile runs: a --VMfile run
    # produces only the A-style VM export below (SXD/ holds VM exports only).
    if not nowrite and not VMfile:
        if outdir is None:
            outdir = SXD_DIR
